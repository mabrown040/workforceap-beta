...
export async function getScoreBreakdown(userId: string): Promise<ScoreBreakdown> {
  const [user, goals, aiResults, resourceProgress, learningProgress, pathwaySteps, jobApps, certs, interviewPracticeCompletionEvent, lastEvent] =
    await Promise.all([
      ...
      prisma.aIToolResult.findMany({ take: 5000, where: { userId }, select: { toolType: true, createdAt: true } }),
      ...
      prisma.jobApplication.findMany({ take: 5000, where: { userId } }),
...
export async function getScoreBreakdowns(userIds: string[]): Promise<Map<string, ScoreBreakdown>> {
  ...
  const [users, goals, aiResults, resourceProgress, learningProgress, pathwaySteps, jobApps, certs, interviewEvents, lastEvents] =
    await Promise.all([
      ...
      prisma.goal.findMany({ take: 5000, where: { userId: { in: userIds } } }),
      prisma.aIToolResult.findMany({ take: 5000, where: { userId: { in: userIds } }, select: { userId: true, toolType: true, createdAt: true } }),
      prisma.resourceProgress.findMany({ take: 5000, where: { userId: { in: userIds } } }),
      prisma.learningProgress.findMany({ take: 5000, where: { userId: { in: userIds } } }),
      prisma.pathwayStepProgress.findMany({ take: 5000, where: { userId: { in: userIds } } }),
      prisma.jobApplication.findMany({ take: 5000, where: { userId: { in: userIds } } }),
      prisma.userCertification.findMany({ take: 5000, where: { userId: { in: userIds } } }),
