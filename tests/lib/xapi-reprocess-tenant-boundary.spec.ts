...
describe('xAPI reprocess tenant boundary', () => {
  ...
  it('does not auto-heal an ILIKE neighbor when the actor address itself is absent', async () => {
    mocks.queryRaw
      .mockResolvedValueOnce([{ ...event, actor_email: 'jane_doe@example.com' }])
      .mockResolvedValueOnce([{ status: 'unmatched' }]);
    mocks.findMany.mockResolvedValue([
      { id: 'neighbor', email: 'jane.doe@example.com', organizationId: 'org-a' },
    ]);
    mocks.handle.mockResolvedValue({ completions: [] });
