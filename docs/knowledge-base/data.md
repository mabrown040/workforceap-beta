...
| Enrollment and curriculum version | `CourseEnrollment`; [curriculum assignment](../../lib/member/curriculumAssignment.ts) | Preserve organization, program slug, curriculum version, primary selection, funding and provenance. Legacy `User.enrolledProgram` is not the complete assignment model. |
| Training evidence vs projection | `XapiStatement`, Coursera mapping/progress records, `CourseProgress`, `MemberProgramProgress` | Inbound evidence, identity resolution and displayed normalized progress are different stages. Replay must preserve idempotency and delivery accounting. |
