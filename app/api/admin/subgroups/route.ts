...
export const GET = withApiGuc(_GET);async function _POST(request: NextRequest) {
  ...
  void auditLog({ actorUserId: user.id, action: 'admin_subgroup_create', targetType: 'subgroup', targetId: subgroup.id, metadata: { name: subgroup.name, type: subgroup.type } }).catch(() => {});
