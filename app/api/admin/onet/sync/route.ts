...
async function _POST(request: NextRequest) {
  ...
  if (allMapped) {
    ...
    void auditLog({ actorUserId: user.id, action: 'admin_onet_sync', targetType: 'organization', targetId: user.id, metadata: { allMapped: true, synced, errorCount: errors.length } }).catch(() => {});
  ...
  if (onetCodes?.length) {
    ...
    void auditLog({ actorUserId: user.id, action: 'admin_onet_sync', targetType: 'organization', targetId: user.id, metadata: { onetCodes, synced: ok, errorCount: errors.length } }).catch(() => {});
