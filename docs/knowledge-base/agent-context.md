...
- **Training:** `lib/content/programs.ts` is the app catalog, not the only catalog. Assignment is `CourseEnrollment` via `lib/member/courseEnrollmentAssignment.ts` + `lib/member/curriculumAssignment.ts`. `User.enrolledProgram` is a leftover pointer. Coursera/xAPI: `lib/coursera/`, `lib/xapi/`.
