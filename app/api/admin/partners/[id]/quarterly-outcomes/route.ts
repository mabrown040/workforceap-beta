...
export const GET = withApiGuc(async (req: NextRequest, { params }: { params: Promise<{ id: string }> }) => {
  try {
    ...
    const body = await generatePartnerQuarterlyOutcomes(orgId, partnerId, spec);

    return NextResponse.json(body);
  } catch (error) {
    console.error('/api/admin/partners/[id]/quarterly-outcomes error:', error);
