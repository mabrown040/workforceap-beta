  ...
  try {
    ...
    try {
      ...
      logAuditEvent({
        ...
        verb: 'edit_profile',
        object: { type: 'User', id },
        result: { success: true, extensions: { fields: Object.keys(parsed.data).filter(k => (parsed.data as Record<string, unknown>)[k] !== undefined) } },
        request: auditRequestMeta(req),
        orgId,
      }).catch((err) => console.error('[audit] edit_profile:', err));
