# Portal Kit Guide (DRAFT)

> **Status: DRAFT** — proposed agent-facing guide, written as part of the Astryx pattern-mining
> study (`docs/ASTRYX_LESSONS.md`, Lesson 3). Once reviewed, rename to `docs/KIT_GUIDE.md`, link
> it from `AGENTS.md`, and add a pointer comment in `components/portal/kit/index.ts`.
...
There are **three** CSS-variable families in this codebase. Only one is canonical for new work.

| Family | File | Status |
|---|---|---|
| `--wa-*` | `css/portal-tokens.css` | **Canonical.** Use this for all kit/portal work. |
| `--color-*`, `--surface-container-*` | `css/main.css` | Legacy (MD3-style). Do not add new refs; bridge aliases in `portal-tokens.css:49-57` map the live names onto `--wa-*`. Caution: its `:root` holds **dark** values with light as the override — the opposite convention from `--wa-*`. |
| `--dm-*` | `css/dark-mode.css` | Dead. Never use. (Slated for deletion.) |
...
Tailwind is configured with the **`wa-` prefix** (`tailwind.config.ts:15`) — utilities look like
...
that the token layer reads (`css/portal-tokens.css:103-119`). Brand colors are identical in both;
...
- Toggled by `html.dark` (system or explicit) and `[data-theme='dark']` (explicit), set by
  `components/theme/ThemeInitScript.tsx`. The `--wa-*` neutrals and `-soft` tints flip
  (`css/portal-tokens.css:128-144`); brand hues stay constant.
- **Never** hand-write a dark variant of a color. If you consume `--wa-*` tokens, dark mode is
  ...
  the pattern in `lib/ui/statusColors.ts`).
- Elevation in dark mode comes from shadows (`--wa-shadow-lg` deepens), not from lighter surface
  tones. Do not add new surface-tone variables; `--wa-surface` and `--wa-surface-2` are the whole
  ladder.
...
**Kit `KitTone`** (`components/portal/kit/tokens.ts:28`) — used by `<StatusTag tone=…>` via
...
brand-magenta highlight — use `danger` there (see the note in `tokens.ts:22-27`).
...
## 5. Component index (`components/portal/kit/index.ts`)
...
- Respect reduced motion: gate animations/transitions on
  `@media (prefers-reduced-motion: reduce)` (the token layer already downgrades `--wa-shadow-lg`,
  `css/portal-tokens.css:147-151`).
...
1. **Inventing a fourth token family** or referencing `--dm-*`. Use `--wa-*`.
...
3. **Hand-writing dark-mode overrides** per component instead of consuming tokens / alpha tints.
...
5. **Raw `<table>` + manual borders** instead of `DataTable`; ad-hoc stat blocks instead of
   `StatTile`/`KpiStrip`.
6. **Wrapping single components in `DesignSurface`** — it belongs on route-group layouts.
...
9. **Inventing props.** Read the component source in `components/portal/kit/` first; the kit is
