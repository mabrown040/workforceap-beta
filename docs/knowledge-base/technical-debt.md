...
| --- | --- | --- |
| KB-06 | **26 existing Node suites are outside standard collection at the reviewed baseline.** They span route, email, script and shared-code tests. | Extend collection deliberately, preserve explicit skips/delegation, run newly collected suites and add a collection-coverage gate. [Declared runner inventory](generated/test-runners.json) and [per-file test records](generated/tests.json) locate the current selection. This audit did not execute the omitted suites or prove their product paths fail. |
