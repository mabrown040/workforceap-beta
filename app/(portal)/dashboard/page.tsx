...
export const metadata: Metadata = buildPageMetadata({
  ...
  description:
    "Your WorkforceAP member dashboard — training progress, next steps, career tools, and application status.",
  path: "/dashboard",
});
    ...
    return (
      ...
      >
        ...
        >
          ...
          temporary. Try again, or open another section from the menu.
        </p>
...
async function renderMemberDashboard(
  ...
  const { user: dbUser, careerBrief } = await loadMemberCareerBriefBundleSafe(
    user.id,
  ...
  const [intakeResult, profileResult, engagementResult] =
  ...
  if (profileResult.status === "rejected") {
    console.error(
      "[dashboard] profile completeness query failed",
  ...
  if (engagementResult.status === "rejected") {
    console.error(
      "[dashboard] engagement signals failed",
      engagementResult.reason,
    );
  }

  const careerMatchFromProfile =
    intakeExtra?.careerRecommendationJson as CareerMatchResult | null;

  const [
    toolsResult,
    applicationResult,
    dynamicActionsResult,
  ...
  if (toolsResult.status === "rejected") {
    console.error(
      "[dashboard] recent AI tools query failed",
      toolsResult.reason,
    );
  }
  ...
  if (applicationResult.status === "rejected") {
    console.error(
      "[dashboard] latest application query failed",
      applicationResult.reason,
    );
  }

  const dynamicNextActions =
    dynamicActionsResult.status === "fulfilled"
      ? dynamicActionsResult.value
      : [];
  if (dynamicActionsResult.status === "rejected") {
    console.error(
      "[dashboard] dynamic actions query failed",
      dynamicActionsResult.reason,
    );
  }
  ...
  const hasCompletedInterviewPractice =
    interviewPracticeCompletionResult.status === "fulfilled"
      ? !!interviewPracticeCompletionResult.value
      : false;
  if (interviewPracticeCompletionResult.status === "rejected") {
    console.error(
      "[dashboard] interview practice completion query failed",
      interviewPracticeCompletionResult.reason,
    );
  }
  ...
  const sessionEvents =
    sessionEventsResult.status === "fulfilled" ? sessionEventsResult.value : [];
  if (sessionEventsResult.status === "rejected") {
    console.error(
      "[dashboard] session events query failed",
      sessionEventsResult.reason,
    );
  }
  type SessionSummary = {
    sessionId: string;
  ...
  const applicationStatusView = buildMemberApplicationStatusView(
    latestApplication,
    {
  ...
  const userAge = intakeExtra?.profile?.dob
    ? Math.floor(
        (Date.now() - new Date(intakeExtra.profile.dob).getTime()) /
          (365.25 * 24 * 60 * 60 * 1000),
      )
    : null;
  const isMinor =
    intakeExtra?.profile?.isMinor || (userAge !== null && userAge < 18);

  const program = enrolledProgram ? getProgramBySlug(enrolledProgram) : null;
  const totalCourses = program?.courses.length ?? 0;
  const completedCount = program
    ? coursesCompleted.filter((s) => program.courses.some((c) => c.slug === s))
        .length
    : 0;
  ...
  const profileCompletenessPct = getProfileCompleteness(
  ...
  if (dbUser.enrolledAt) {
    recentActivity.push({
      ...
      timestamp: dbUser.enrolledAt,
    });
  }
  if (dbUser.assessmentCompletedAt) {
    recentActivity.push({
      label: "Completed skills assessment",
      timestamp: dbUser.assessmentCompletedAt,
    });
  }
  ...
  const mobileProgressTone = allCoursesComplete
    ? "Completed"
    : completedCount > 0
      ? "In progress"
      : "Getting started";
  const mobileProgressSummary =
    totalCourses > 0
  ...
  const journeySteps = [
    ...
    },
    {
      ...
      detail: assessmentCompleted
        ? "Completed"
        : enrolledProgram
          ? "Complete to start training"
    ...
    <>
      ...
      <div className="md:wa-hidden portal-mobile-content">
        ...
        <section style={{ padding: "1.25rem 1.25rem 1rem" }}>
          <div
            style={{
              ...
              padding: "1rem",
              background:
                "linear-gradient(180deg, color-mix(in srgb, var(--color-accent) 7%, white) 0%, white 52%)",
              border:
                "1px solid color-mix(in srgb, var(--color-accent) 14%, var(--outline-variant))",
                ...
                {program && (
                  <div
                    style={{
                      ...
                      border:
                        "1px solid color-mix(in srgb, var(--color-accent) 10%, var(--outline-variant))",
                    }}
                  >
                    <p
                      style={{
                ...
                >
                  <div
                    ...
                    style={{
                      ...
                      borderRadius: "999px",
                      background:
                        "radial-gradient(circle at center, color-mix(in srgb, var(--color-accent) 10%, white) 0%, white 60%)",
                      boxShadow:
                        "0 14px 32px color-mix(in srgb, var(--color-accent) 14%, transparent)",
                      border:
                        "1px solid color-mix(in srgb, var(--color-accent) 12%, white)",
                    }}
                  >
                    <svg
                      style={{
                    ...
                    >
                      <span
                        className="wa-text-xl wa-font-extrabold wa-text-[var(--color-accent-dark)]"
                        style={{ lineHeight: 1 }}
                      >
                        {mobilePct}%
                      </span>
                      <span
                        className="wa-text-[9px] wa-font-semibold wa-uppercase wa-tracking-[0.12em] wa-text-[var(--color-on-surface-variant)]"
                        style={{ lineHeight: 1 }}
                      >
                        {mobileProgressTone}
                      </span>
                  ...
                  <div
                    style={{
                      ...
                      border:
                        "1px solid color-mix(in srgb, var(--color-accent) 20%, white)",
                    }}
                  >
            ...
            {dashboardState !== "A" && (
              <div
                style={{
                  ...
                  paddingTop: "0.9rem",
                  borderTop:
                    "1px solid color-mix(in srgb, var(--outline-variant) 78%, white)",
                }}
              >
                <p
                  className="wa-text-xs wa-text-[var(--color-on-surface-variant)]"
                  style={{ margin: 0, lineHeight: 1.5 }}
                >
                  Training progress is based on completed courses. Your
                  application steps are shown below.
                </p>
              </div>
            )}
          </div>
          ...
          <section style={{ padding: "0 1.25rem 1.25rem" }}>
            <Link
              ...
              style={{
                ...
                boxShadow:
                  "0 6px 24px color-mix(in srgb, var(--color-accent) 28%, transparent)",
                padding: "1.25rem",
                textDecoration: "none",
              }}
            >
              ...
              >
                <span>
                  {noApplicationOnFile ? "Start Application" : "Choose Program"}
                </span>
                <span
            ...
            <div
              style={{
                ...
                background:
                  "linear-gradient(135deg, var(--color-accent-dark), var(--color-accent))",
                boxShadow:
                  "0 6px 24px color-mix(in srgb, var(--color-accent) 30%, transparent)",
              }}
            >
              <div
                style={{
              ...
              >
                ...
                >
                  ...
                  <span
                    className="material-symbols-outlined wa-text-xl"
                    style={{ color: "var(--color-gold)", "--ms-fill": 1 }}
                    aria-hidden
                  >
                    bolt
                  </span>
        ...
        <section style={{ padding: "0 1.25rem", marginBottom: "1.5rem" }}>
          <div className="portal-dash-section-header">
            <h3 className="portal-dash-section-header__title">
              Application Journey
            </h3>
          </div>
          <div className="portal-journey-timeline">
            {journeySteps.map((step, i) => {
              ...
              return (
                ...
                >
                  <div
                    className={`portal-journey-step__dot portal-journey-step__dot--${step.done ? "done" : step.active ? "active" : "locked"}`}
                  >
                    {step.done && (
                      <span
                        className="material-symbols-outlined"
                    ...
                    {step.active && !step.done && (
                      <div className="portal-dot-pulse" />
                    )}
                  </div>
                  <div className="portal-journey-step__content">
                    <p
                      className={`portal-journey-step__label${step.active && !step.done ? " portal-journey-step__label--active" : ""}`}
                    >
                      {step.label}
                    </p>
                    {step.detail && (
                      <p className="portal-journey-step__detail">
                        {step.detail}
                      </p>
                    )}
                  </div>
        ...
        {!enrolledProgram ? (
          ...
          >
            ...
            >
              ...
              <a
                href="/programs"
                className="wa-text-xs wa-font-bold wa-text-[var(--color-accent-dark)]"
                ...
                >
                  <div
                    style={{
                      height: "7rem",
                      position: "relative",
                      background: `linear-gradient(135deg, ${prog.categoryColor} 0%, var(--surface-container-highest) 100%)`,
                    }}
                  />
                  <div
                    style={{
                  ...
                  >
                    <p
                      className="wa-text-[11px] wa-font-bold wa-uppercase wa-tracking-widest"
                      style={{ color: "var(--color-gold)" }}
                    >
                      {prog.partner || "WorkforceAP"}
                    </p>
                    <h4 className="wa-font-bold wa-text-sm wa-text-[var(--color-on-surface)] wa-leading-tight">
                      {prog.title}
                    </h4>
                  </div>
                </div>
              ...
              {[
                {
                  ...
                  desc: nextIncompleteCourse?.name
                    ? "Pick up where you left off."
                    : "Open your training track and keep progressing.",
                  href: "/dashboard/training",
                  icon: "school",
                },
                {
                    ...
                    >
                      ...
                      >
                        <div style={{ minWidth: 0 }}>
                          <p
                            className="wa-text-[10px] wa-font-bold wa-uppercase wa-tracking-widest"
                            style={{
                              color: "var(--color-on-surface-variant)",
                              margin: 0,
                            }}
                          ...
                          <p
                            className="wa-text-sm wa-font-bold wa-tracking-tight"
                            style={{
                              color: "var(--color-on-surface)",
                              margin: "0.35rem 0 0",
                            }}
          ...
          <div className="portal-quick-grid-2x2">
            {(
              ...
              <a
                key={action.label}
                href={action.href}
                className="portal-quick-grid-item"
              >
                ...
                <span className="portal-quick-grid-item__label">
                  {action.label}
                </span>
              </a>
            ))}
        ...
        {/* ── Recent career toolkit activity — mobile ── */}
        {recentTools.length > 0 && (
          <section
            style={{ padding: "0 1.25rem", marginBottom: "1.5rem" }}
            aria-label="Recent career toolkit activity"
          >
            <div className="portal-dash-section-header">
              <h3 className="portal-dash-section-header__title">
                Recent Career Toolkit Activity
              </h3>
              <Link
      ...
      <div className="wa-hidden md:wa-block">
        <PortalEntryErrorBoundary>
          <Suspense
            fallback={<PortalLoadingState message="Loading portal..." />}
          >
            <PortalEntryClient
              portal="member"
              ...
              wizardProps={{
                ...
                initialPhone:
                  intakeExtra?.profile?.profilePhone ??
                  intakeExtra?.phone ??
                  "",
                ...
                initialReferralSource:
                  intakeExtra?.profile?.referralSource ?? "",
              }}
            >
              <div
                style={{
              ...
              <Suspense
                fallback={<PortalLoadingState message="Loading dashboard..." />}
              >
                <DashboardHomeClient
                  ...
                  interviewRequestedAt={
                    intakeExtra?.interviewRequestedAt ?? null
                  }
                  interviewCompletedAt={
                    intakeExtra?.interviewCompletedAt ?? null
                  }
                  starterProfileReviewRequired={starterProfileReview.required}
                  starterProfileMissingFields={starterProfileMissingLabels}
                  state={dashboardState}
              ...
              {showMatchedRoles && userAge !== null && userAge < 14 ? null : (
                <Suspense
                  fallback={
                    <PortalLoadingState message="Loading career matches..." />
                  }
                >
                  <MatchedRoles />
                </Suspense>
