...
```
Pages that already call `getUser()` and then Prisma should wrap only the Prisma section. Pages with multiple parallel queries should wrap all of them in a single `withAuthGuc` call (it's idempotent).
