...
// Master's route verifies the live app account before the rate limit; without
// this mock every request would 403 before reaching the session helper.
vi.mock('@/lib/ai/voiceSessionBoundary', () => ({
  hasActiveVoiceSessionUser: vi.fn(async () => true),
  VOICE_SESSION_IDENTITY_MESSAGE: 'identity',
  VOICE_SESSION_RESPONSE_HEADERS: { 'Cache-Control': 'no-store, max-age=0' },
}));
