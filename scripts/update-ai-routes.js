...
for (const route of AI_ROUTES) {
  ...
  // 1. Add api-utils imports after the last import block
  const apiUtilsImports = [
  ...
  if (!content.includes("from '@/lib/api-utils'")) {
    // Find a good insertion point - after the last import line before the code
    ...
    if (lastImportIdx >= 0) {
      lines.splice(lastImportIdx + 1, 0, ...apiUtilsImports);
      content = lines.join('\n');
    }
  }

  // 2. Add rate-limit imports for the new checkers if not present
  if (!content.includes('checkAICoachUserRateLimit')) {
    const rateLimitImport = "import { checkAICoachUserRateLimit, checkAICoachIpRateLimit } from '@/lib/rate-limit';";
    if (content.includes("from '@/lib/rate-limit'")) {
      // Add to existing rate-limit import line
      ...
      if (lastImportIdx >= 0) {
        lines.splice(lastImportIdx + 1, 0, rateLimitImport);
        content = lines.join('\n');
      }
    }
  }
  ...
  content = content.replace(
    /return NextResponse\.json\(\n\s*\{ error: parsed\.error\.errors\[0\]\?\.message \?\? 'Validation failed' \},\n\s*\{ status: 400 \}\n\s*\);/g,
    "return createApiErrorResponse(parsed.error.errors[0]?.message ?? 'Validation failed', 'VALIDATION_ERROR', 400);"
  );

  // 500 Internal server error (catch blocks)
  content = content.replace(
  ...
  // Look for pattern: const { success } = await checkAIToolRateLimit(user.id);
  // and add the dual check after it
...
console.log('Done updating AI routes');
