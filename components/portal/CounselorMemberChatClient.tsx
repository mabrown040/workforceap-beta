...
export default function CounselorMemberChatClient({ initial }: { initial: InitialPayload }) {
  ...
  const markRead = useCallback(async () => {
    try {
      await fetch(`/api/counselor/members/${member.id}/messages`, { method: 'PATCH', credentials: 'include' });
    } catch {
      /* ignore */
    }
  }, [member.id]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const channel = supabase
        .channel(`counselor-thread:${threadId}`)
        .on(
          'postgres_changes',
      ...
      console.warn('[CounselorMemberChat] Realtime unavailable', e);
      return undefined;
    }
  ...
  const send = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const r = await fetch(`/api/counselor/members/${member.id}/messages`, {
        method: 'POST',
        credentials: 'include',
  ...
  return (
    <div className="admin-member-counselor-chat">
      ...
      <form className="member-counselor-chat__form" onSubmit={send}>
        <label htmlFor="counselor-chat-input" className="sr-only">
          Reply
        </label>
        <textarea
