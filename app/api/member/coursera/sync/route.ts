...
export async function POST() {
  ...
  const catalogHealth = getProgramCourseCatalogHealth(program);
  ...
  try {
    ...
    const completedSlugsPredicted = skillsetMerge.courseSlugs;

    const mergedCourses: Array<{ courseSlug: string; ok: boolean; alreadyCompleted?: boolean; error?: string }> = [];

    for (const slug of completedSlugsPredicted) {
      try {
