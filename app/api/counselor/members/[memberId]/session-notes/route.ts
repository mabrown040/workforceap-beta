...
async function _POST(
  ...
  try {
    ...
    auditLog({
      actorUserId: user.id,
      action: 'counselor_session_note_created',
      targetType: 'User',
      targetId: memberId,
    ...
    logAuditEvent({
      ...
      verb: 'created',
      object: { type: 'AdvisorSessionNote', id: note.id },
      result: { success: true },
    }).catch(() => {});
...
async function _DELETE(
  ...
  try {
    ...
    auditLog({
      actorUserId: user.id,
      action: 'counselor_session_note_deleted',
      targetType: 'User',
      targetId: memberId,
      metadata: { noteId },
    }).catch(() => {});
    logAuditEvent({
      user: { id: user.id, role: 'counselor' },
      verb: 'deleted',
      object: { type: 'AdvisorSessionNote', id: noteId },
      result: { success: true },
    }).catch(() => {});
