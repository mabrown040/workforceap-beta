...
| `npm run check-migrations` | Required early CI guard for new timestamp collisions and exact historical directory/SQL exceptions. [Checker](../../scripts/check-duplicate-migrations.mjs), [pinned exceptions](../../scripts/migration-collision-baseline.json) and [recovery limits](../DATABASE-RECOVERY.md#repository-migration-collision-gate). Does not prove SQL safety or clean replay. |
