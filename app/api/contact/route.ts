...
export async function POST(request: NextRequest) {
  ...
  const { firstName, lastName, email, phone, topic, message, preferSms } = parsed;
  ...
  const text = [
    ...
    `Prefer contact by text: ${preferSms ? 'Yes' : 'No'}`,
    `Topic: ${topic}`,
    '',
    'Message:',
    message,
...
function parseBody(body: unknown): {
  ...
  const preferSms = o.prefer_sms === true || o.prefer_sms === 'true';
  if (!firstName || !lastName || !email || !topic || !message) return null;
  return { firstName, lastName, email, phone, topic, message, preferSms };
}
