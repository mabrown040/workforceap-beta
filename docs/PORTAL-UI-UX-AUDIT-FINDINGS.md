...
> **Current-state note (2026-08-31):** `/dashboard/counselor` is Lilley, the member AI career coach. Member/account context is fetched through three tenant-scoped, read-only tools; browser-controlled text is not inserted into Lilley's system prompt. It is not staff or caseload tooling. Historical snapshots below retain the wording that existed when this audit was run.
