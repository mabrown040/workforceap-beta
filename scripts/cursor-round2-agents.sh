#!/bin/bash
# Round 2 — Launch Cursor agents to fix audit findings

API_KEY="${CURSOR_API_KEY:-crsr_3fd6d185a558e95bb7dd2e23d7f274b726588bbf8b976227355b34ee43ec7305}"

cd /home/mike/.openclaw-dench/workspace/wap-repo || exit 1

echo "=== Launching Round 2 Cursor Agents ==="
echo ""

# Agent 1: Member Trust + Mobile UX
echo "[1/5] Launching Member Trust + Mobile UX..."
cursor run \
  --task "You are a member-first UX engineer for WorkforceAP — a nonprofit career training platform serving low-income adults, many on cracked Android phones with tight data and skepticism about 'free' promises.
...
- PR title: 'fix(member): mobile font sizes, trust copy, and honest error states'" \
  --auto-apply \
  --api-key "$API_KEY" \
  2>&1 | tee /tmp/cursor-r2-agent1.log &

# Agent 2: Partner Pipeline Truth
echo "[2/5] Launching Partner Pipeline Truth..."
cursor run \
  --task "You are a data-integrity engineer for WorkforceAP. Partners (WIOA, nonprofits) rely on accurate pipeline counts to report outcomes and manage referrals.
...
- PR title: 'fix(partner): pipeline count accuracy, training completion visibility, and honest labels'" \
  --auto-apply \
  --api-key "$API_KEY" \
  2>&1 | tee /tmp/cursor-r2-agent2.log &

# Agent 3: Employer Data Accuracy
echo "[3/5] Launching Employer Data Accuracy..."
cursor run \
  --task "You are a data-integrity engineer for WorkforceAP. Employers pay to post jobs and find candidates. Data accuracy is revenue-adjacent.
...
- PR title: 'fix(employer): separate matches from applications, honest labels, and training filters'" \
  --auto-apply \
  --api-key "$API_KEY" \
  2>&1 | tee /tmp/cursor-r2-agent3.log &

# Agent 4: Spanish i18n Sweep
echo "[4/5] Launching Spanish i18n Sweep..."
cursor run \
  --task "You are an i18n engineer for WorkforceAP. Spanish-speaking members are a significant portion of our user base. The portal must feel native in Spanish.
...
- PR title: 'fix(i18n): complete spanish translations for employer, partner, counselor portals'" \
  --auto-apply \
  --api-key "$API_KEY" \
  2>&1 | tee /tmp/cursor-r2-agent4.log &

# Agent 5: CEO Strategic Fixes
echo "[5/5] Launching CEO Strategic Fixes..."
cursor run \
  --task "You are a strategic engineer for WorkforceAP. The CEO review scored the product 6.4/10 with biggest risks around training truth under load, calendar diligence, and employer job API verification.
...
- PR title: 'feat(ops): add stale-data honesty, pipeline integrity checks, and WIOA metrics'" \
  --auto-apply \
  --api-key "$API_KEY" \
  2>&1 | tee /tmp/cursor-r2-agent5.log &

echo ""
echo "All 5 Round 2 agents launched in background."
echo "Logs: /tmp/cursor-r2-agent{1-5}.log"
echo ""
echo "Check status with: ps aux | grep 'cursor run'"
