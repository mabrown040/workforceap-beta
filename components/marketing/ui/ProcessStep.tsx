...
export function ProcessStep({ step, icon, title, description, centered = false }: ProcessStepProps) {
  return (
    <div style={{ textAlign: centered ? 'center' : 'left', padding: '0 1rem', position: 'relative', zIndex: 1 }}>
      <div
        className={`marketing-num-pill__index btn-radius-full marketing-process-step__badge${centered ? ' marketing-process-step__badge--centered' : ' marketing-process-step__badge--left'}`}
      >
        {step}
      </div>
      {icon && (
