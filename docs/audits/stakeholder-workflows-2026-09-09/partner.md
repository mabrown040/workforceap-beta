...
- **Visual limits:** the guide’s existing resource layout and other legacy panels remain. Browser review reproduced a preexisting mobile chat bug: `scrollIntoView()` scrolled the outer page so most of the conversation sat above the viewport. The bounded follow-up now scrolls only the message log, preserving reduced-motion preferences. Two regressions failed on the old behavior and pass after the fix; the rebuilt browser check is pending.
