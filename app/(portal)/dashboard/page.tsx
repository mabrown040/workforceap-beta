...
async function renderMemberDashboard(
  ...
  // v2 KIT is the DEFAULT dashboard; legacy stays at ?ui=legacy.
  // Kit path now also: (1) wraps first-run onboarding/tour, (2) pulls B4B
  // progress + getMemberState for stage-aware "Do this next", (3) fires
  // Coursera identity auto-link in the background. Chrome queries stay lean.
  if (args.requestedUi !== 'legacy') {
    ...
    // Coursera truth: bind portal email → identity, backfill orphans, seed
    // progress when empty. Non-blocking — same pattern as the legacy path.
    void maybeAutoSyncCourseraOnDashboard({
      userId: user.id,
      userEmail: user.email ?? null,
    }).catch((err) => {
      console.warn('[dashboard] kit path Coursera auto-sync failed:', err);
    });

    // Resolve active program first so B4B can be scoped to one programId.
    const kitActiveProgram = await getActiveProgramForDashboard({
      userId: user.id,
      requestedProgramSlug: args.requestedProgramSlug,
    }).catch((err) => {
      console.warn('[dashboard] kit active-program resolve failed:', err);
      return null;
    });
    const leanSlug =
      kitActiveProgram?.activeProgramSlug ??
      ku?.enrolledProgram ??
      null;
    const courseraProgramIdForKit = leanSlug
      ? DISCOVERED_COURSERA_PROGRAMS[leanSlug]?.courseraProgramId
      : undefined;

    const b4bProgressPromise: Promise<LearnerProgressByContent> = user.email
      ? fetchLearnerProgressFromB4B(user.email, {
          programId: courseraProgramIdForKit,
        }).catch((err) => {
          console.warn('[dashboard] kit B4B learner progress unavailable:', err);
    ...
    const b4bProgress = await Promise.race([
      ...
      new Promise<LearnerProgressByContent>((resolve) =>
        setTimeout(() => {
          console.warn('[dashboard] kit B4B exceeded 4s — using local fallback');
          resolve(new Map() as LearnerProgressByContent);
        }, 4000),
      ),
    ]);

    // Stage-aware coaching (cached 300s). Fail-soft → kit still renders chrome.
    const memberState = await getMemberState(user.id, {
      b4bProgress,
      activeProgramSlug: leanSlug,
    }).catch((err) => {
      console.warn('[dashboard] kit getMemberState failed:', err);
      return null;
    });

    // Cheap chrome queries for kit tiles (points, pipeline, goals, ledger).
    const [leanAiCount, leanActions, leanCertCount, leanPointsRow, leanActiveJobs, leanPipeline, leanGoals, leanRecentPoints, leanWeekPoints] = await withDbRetry(() => Promise.all([
    ...
    void leanAiCount; // reserved for future kit AI-usage tile
    const leanProgram = leanSlug ? getProgramBySlug(leanSlug) : undefined;
    // Prefer B4B-blended training view from getMemberState; fall back to local counts.
    const leanCompleted = memberState?.trainingView?.completedCount
      ?? (leanSlug
        ? await withDbRetry(() =>
        ...
        : 0);
    const leanTotal =
      memberState?.trainingView?.totalCourses
      ?? leanProgram?.courses?.length
      ?? 0;
    const leanPct =
      memberState?.trainingView?.progressPercentDisplay
      ?? (leanTotal ? Math.round((leanCompleted / leanTotal) * 100) : 0);
    const firstNameLean = (ku?.fullName ?? user.email ?? 'there').split(' ')[0] || 'there';

    // Prefer counselor/system-authored pending NBA; else stage-aware coaching.
    const topLeanAction = leanActions[0] ?? null;
    const stageAction = memberState?.nextBestActions?.[0] ?? null;
    ...
    const nextLessonDue = topLeanAction || stageAction
      ? 'Recommended next step'
      : 'Up next';
    const resumeHref =
      topLeanAction?.ctaHref
      ?? stageAction?.href
      ?? programHref;
    ...
    const leanPointsLedger = leanRecentPoints.map((tx) => ({
      label: EVENT_LABELS[tx.event] ?? 'Points earned',
      amount: tx.points,
      color: pointsLedgerColor(tx.event),
    }));
    ...
    const showMemberOnboarding = ku?.onboardingCompletedAt == null;
    const showMemberTour =
      ku?.onboardingCompletedAt != null && ku?.tourCompletedAt == null;
    const superAdmin = await isSuperAdmin(user.id).catch(() => false);
    const wizardProgramInterest =
      memberState?.application?.programInterest ?? ku?.programInterest ?? '';
    ...
    return (
      <PortalEntryErrorBoundary>
        <PortalEntryClient
          ...
          wizardProps={{
            initialFullName: ku?.fullName ?? '',
            initialPhone: ku?.profile?.profilePhone ?? ku?.phone ?? '',
            initialAddress: ku?.profile?.profileAddress ?? '',
            initialCity: ku?.profile?.city ?? '',
            initialState: ku?.profile?.state ?? '',
            initialZip: ku?.profile?.zip ?? '',
            initialProgramInterest: wizardProgramInterest,
            initialReferralSource: ku?.profile?.referralSource ?? '',
            initialStep: ku?.onboardingCurrentStep ?? 0,
          }}
        >
          <MemberHomeKit
